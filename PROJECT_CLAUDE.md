...
For Reuters Graphics- and kit-specific terminology, read [`.claude/context/glossary.md`](.claude/context/glossary.md).

## Tech stack

- **SvelteKit** with **Svelte 5** — always use Svelte 5 runes syntax (`$state`, `$derived`, `$effect`, etc.)
- **TypeScript** preferred throughout
- **SCSS** for styles — see [`.claude/context/styling.md`](.claude/context/styling.md) for conventions on using the design system tokens
...
For detail on how CMS content is structured and how to wire it into graphics components in `App.svelte`, read [`.claude/context/content.md`](.claude/context/content.md). For working with static assets and media files, read [`.claude/context/assets.md`](.claude/context/assets.md). For routing conventions, page and embed structure, and path aliases, read [`.claude/context/routing.md`](.claude/context/routing.md).
...
For the publishing workflow and safety rules around `pnpm upload` and `pnpm pub`, read [`.claude/context/publishing.md`](.claude/context/publishing.md).
