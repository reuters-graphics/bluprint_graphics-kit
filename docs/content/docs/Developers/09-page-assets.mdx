...
So to use media files in your component code, you need to resolve the path to them using SvelteKit's [built-in `asset` function](https://svelte.dev/docs/kit/$app-paths#asset). Here's how:
...
<img alt="" src={asset('/images/my-image.jpg')} />
```
...
<div
  style="background-image: url({asset('/images/my-image.jpg')});"
></div>
